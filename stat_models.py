...
class StatisticalHeatwavePredictor:
    ...
    def prepare_annual_counts(self):
        ...
        self.annual_counts = self.data[self.data['Heatwave']].groupby(
        ...
        self.annual_counts['Year_Log'] = np.log(self.annual_counts['Year'] - start_year + 1)
        
        print(f"✅ Prepared {len(self.annual_counts)} years of annual heatwave data")
        print(f"📅 Period: {self.annual_counts['Year'].min()}-{self.annual_counts['Year'].max()}")
        print(f"📈 Mean heatwave days/year: {self.annual_counts['HeatwaveDays'].mean():.1f}")
        print(f"📊 Std deviation: {self.annual_counts['HeatwaveDays'].std():.1f}")
        print(f"🔢 Range: {self.annual_counts['HeatwaveDays'].min()}-{self.annual_counts['HeatwaveDays'].max()} days")
        ...
        if dispersion_ratio > 1.5:
            print("• ⚠️  Data shows OVERDISPERSION - Negative Binomial may be preferred")
        elif dispersion_ratio < 0.8:
            print("• ⚠️  Data shows UNDERDISPERSION - Consider zero-inflated models")
        else:
    ...
    def fit_poisson_regression(self):
        ...
        if not STATSMODELS_AVAILABLE:
            print("❌ statsmodels required for Poisson regression")
        ...
        try:
            ...
            for i, coef in enumerate(poisson_results.params):
                print(f"• {X.columns[i]}: {coef:.4f} (p={poisson_results.pvalues[i]:.4f})")
    ...
    def fit_negative_binomial_regression(self):
        ...
        if not STATSMODELS_AVAILABLE:
            print("❌ statsmodels required for Negative Binomial regression")
        ...
        try:
            ...
            for i, coef in enumerate(nb_results.params[:-1]):  # Exclude alpha
                print(f"• {X.columns[i]}: {coef:.4f} (p={nb_results.pvalues[i]:.4f})")
    ...
    def _fit_manual_poisson(self):
        ...
        # Optimize
        result = minimize(poisson_log_likelihood, initial_params, args=(X, y), method='BFGS')
        
        if result.success:
            params = result.x
            print(f"✅ Manual Poisson fit successful!")
            ...
            future_X = np.column_stack([
                np.ones(len(future_years)),
                future_trend,
                future_trend ** 2
            ])
            
            future_pred = np.exp(future_X @ params)
            self.predictions['poisson'] = dict(zip(future_years, future_pred))
            
        else:
            print("❌ Manual Poisson fit failed")
    ...
    def _fit_manual_negbinom(self):
        ...
        if 'poisson' in self.predictions:
            ...
            overdispersion_factor = self.annual_counts['HeatwaveDays'].var() / self.annual_counts['HeatwaveDays'].mean()
            ...
            print(f"✅ Applied overdispersion correction (factor: {overdispersion_factor:.2f})")
    ...
    def _predict_future_poisson(self, results):
        ...
        future_X = pd.DataFrame({
            'const': 1,
            'TimeTrend': future_trend,
            'Year_Squared': future_trend ** 2
        })
    ...
    def _predict_future_negbinom(self, results):
        ...
        # Predictions
        future_pred = results.predict(future_X)
        self.predictions['negative_binomial'] = dict(zip(future_years, future_pred))
    ...
    def _calculate_goodness_of_fit(self, model_name, y_true, y_pred):
        ...
        deviance = 2 * np.sum(y_true * np.log(np.maximum(y_true / np.maximum(y_pred, 1e-10), 1e-10)))
    ...
    def compare_models(self):
        ...
        if len(self.predictions) < 2:
            print("❌ Need both models fitted for comparison")
            return
        ...
        if 'poisson' in self.goodness_of_fit and 'negative_binomial' in self.goodness_of_fit:
            poisson_aic = getattr(self.models.get('poisson', {}).get('results'), 'aic', float('inf'))
            nb_aic = getattr(self.models.get('negative_binomial', {}).get('results'), 'aic', float('inf'))
            ...
            if nb_aic < poisson_aic - 2:  # Substantial improvement
                ...
                print("⚖️  Both models perform similarly - choose based on interpretability")
    ...
    def plot_model_comparison(self, figsize=(16, 12)):
        ...
        fig.suptitle('Statistical Models: Heatwave Prediction Analysis (2025-2030)', 
        ...
        ax1.scatter(years, actual, alpha=0.7, s=60, color='steelblue', 
        ...
        for model_name in ['poisson', 'negative_binomial']:
            if model_name in self.models and 'results' in self.models[model_name]:
                ...
                ax1.plot(years, fitted, '--', linewidth=2, alpha=0.8,
        ...
        for model_name in ['poisson', 'negative_binomial']:
            if model_name in self.predictions:
                pred_values = [self.predictions[model_name][year] for year in future_years]
                ax2.plot(future_years, pred_values, 'o-', linewidth=3, markersize=8,
                        label=f'{model_name.title()} Model', alpha=0.8)
    ...
    def get_prediction_summary(self):
        ...
        if len(self.predictions) >= 2:
            summary += f"\n🏆 MODEL COMPARISON:\n"
            poisson_avg = np.mean(list(self.predictions['poisson'].values())) if 'poisson' in self.predictions else 0
            nb_avg = np.mean(list(self.predictions['negative_binomial'].values())) if 'negative_binomial' in self.predictions else 0
        ...
        summary += f"\n💡 INTERPRETATION:\n"
        summary += f"• Count models are well-suited for heatwave day predictions\n"
        summary += f"• Models capture both trend and natural variability\n"
        summary += f"• Confidence intervals reflect uncertainty in extreme events\n"
...
def run_statistical_analysis(data):
    ...
    # Compare models
    recommended_model = predictor.compare_models()
