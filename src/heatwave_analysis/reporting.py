...
The raw series contained {q['rows']:,} consecutive daily dates; 2024 was incomplete overall but complete for March–June. Mean Tmax was {q['mean_tmax']:.2f} °C and the observed maximum was {q['maximum_tmax']:.1f} °C. The operational definition identified {q['operational_days']} days, whereas requiring at least three consecutive days identified {q['primary_days']} days in {q['primary_events']} events. Descriptive statistics and distributions are reported first in Tables 1–4 and Figures 1–3. Raw hot-season correlations changed in magnitude after removal of the calendar-day climatology, indicating that shared seasonality explained part of several unadjusted relationships.

Across complete annual series and complete hot seasons, trend estimates were outcome-dependent. March–June mean Tmax increased by {primary.ols_hac_slope_per_decade:.3f} °C per decade (95% CI {primary.ols_hac_ci_lower:.3f} to {primary.ols_hac_ci_upper:.3f}). Parametric and nonparametric sensitivity estimates are shown together in Table 9.

The count distribution was assessed before regression. Comparison of Poisson and NB2 supported the **{selected}** model under the prespecified dispersion/AIC rule. Persistent March–June heatwave-day incidence changed by a factor of {count.incidence_rate_ratio:.3f} per decade (95% CI {count.irr_ci_lower:.3f} to {count.irr_ci_upper:.3f}). Residual, influence, and leave-one-year-out results are presented in Figure 6 and Table 13; influential years were not deleted from the primary estimate.

After adjustment for seasonal Fourier terms and long-term trend, per-SD antecedent associations were heterogeneous (Table 15, Figure 7). These are predictive associations, not meteorological causal effects. Blocked validation provided mean full-model precision–recall AUC {binary[binary.model=='antecedent_full'].precision_recall_auc.mean():.3f} and Brier score {binary[binary.model=='antecedent_full'].brier_score.mean():.3f}; comparison with the seasonal-trend base model is in Table 16.

In rolling-origin temperature validation, {best} had the lowest mean RMSE ({fc.loc[best,'rmse']:.3f} °C), compared with {naive.rmse:.3f} °C for seasonal naive and {fc.loc['monthly_climatology','rmse']:.3f} °C for climatology. Coverage departures in Table 19 caution against precise long-range predictions. Accordingly, 2025–2029 results are labeled conditional trend scenarios with wide count intervals, not deterministic forecasts. Five-year forecast skill is not established.
""",encoding="utf-8")
...
| Tmin warmed faster than Tmax | Formal slope interaction | Recomputed in Table 9 | dependent on model | Report interaction CI, not separate significance | The estimated slope difference is reported with its confidence interval. |
...
| Apparent temperature reached ~56.6 °C | Simultaneous temperature/humidity exposure validation | Daily Tmax was paired with daily mean RH | partially supported | Label exploratory calculation | A non-time-matched daily heat-index approximation produced an extreme value. |
