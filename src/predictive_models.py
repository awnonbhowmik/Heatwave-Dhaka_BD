...
class HeatwavePredictor:
    ...
    def fit_arima_model(self):
        ...
        try:
            ...
            plt.plot(
                future_dates,
                forecast,
                label="ARIMA Forecast (2025-2029)",
                color="red",
                linewidth=2,
            )
            ...
            plt.title(
                "Temperature Forecast using ARIMA Model", fontsize=16, fontweight="bold"
            )
            plt.xlabel("Date")
    ...
    def fit_sarima_model(self):
        ...
        try:
            ...
            decomposition.observed.plot(
                ax=axes[0], title="Original Monthly Temperature Series", color="blue"
            )
            ...
            decomposition.trend.plot(
                ax=axes[1], title="Long-term Trend Component", color="red"
            )
            ...
            decomposition.seasonal.plot(
                ax=axes[2], title="Seasonal Component (Annual Cycle)", color="green"
            )
            ...
            decomposition.resid.plot(
                ax=axes[3], title="Residual Component", color="orange"
            )
            ...
            axes[0].set_title(
                "SARIMA Temperature Forecast with Seasonal Patterns",
                fontsize=16,
                fontweight="bold",
            )
            ...
            axes[1].set_title(
                "Seasonal Forecast Pattern vs Historical",
                fontsize=14,
                fontweight="bold",
            )
            ...
            print(
                f"🌡️  Projected warming: {forecast.mean() - historical_avg:.2f}°C by 2025-2029"
            )
    ...
    def fit_lstm_model(self):
        ...
        try:
            ...
            print("Time series data preparation for LSTM...")
            ...
            print(
                f"Building model with input shape: (None, {sequence_length}, {len(available_features)})"
            )
            ...
            model = Sequential(
                [
                    ...
                    # Third LSTM layer
                    LSTM(
                        25, return_sequences=False, dropout=0.2, recurrent_dropout=0.2
                    ),
                    BatchNormalization(),
                    # Dense layers
            ...
            train_pred_original = target_scaler.inverse_transform(
                train_pred.reshape(-1, 1)
            ).flatten()
            test_pred_original = target_scaler.inverse_transform(
            ...
            train_rmse = np.sqrt(
                mean_squared_error(y_train_original, train_pred_original)
            )
            ...
            # Loss curves
            axes[0, 0].plot(
                history.history["loss"], label="Training Loss", color="blue"
            )
            axes[0, 0].plot(
                history.history["val_loss"], label="Validation Loss", color="red"
            )
            ...
            axes[0, 1].plot(
                history.history["val_mae"], label="Validation MAE", color="red"
            )
            ...
            axes[1, 0].scatter(
                y_test_original, test_pred_original, alpha=0.6, color="purple"
            )
            axes[1, 0].plot(
            ...
            plt.title(
                "LSTM Temperature Predictions vs Actual (Last 200 Days)",
                fontsize=14,
                fontweight="bold",
            )
            plt.xlabel("Date")
            ...
            last_sequence = scaled_features[-sequence_length:].reshape(
                1, sequence_length, len(available_features)
            )
            ...
            print("Generating 60-month (5-year) forecast...")
            ...
            future_dates = pd.date_range(
                start=last_date + pd.DateOffset(months=1), periods=60, freq="M"
            )
            ...
            for i, feature in enumerate(available_features):
                ...
                permuted_pred_original = target_scaler.inverse_transform(
                    permuted_pred.reshape(-1, 1)
                ).flatten()
    ...
    def generate_future_predictions(self, years=5):
        ...
        if "random_forest" in self.models:
            ...
            recent_deforest_avg = (
                self.tree_loss_by_year["umd_tree_cover_loss__ha"].tail(5).mean()
            )

            for year in future_years:
                # Use recent temperature patterns and projected deforestation
                temp_prediction = (
                    self.data["Dhaka Temperature [2 m elevation corrected]"]
                    .tail(365)
                    .mean()
                )
                # Add warming trend
    ...
    def plot_predictions(self):
        ...
        if "arima" in self.forecasts:
            ...
            axes[0, 0].set_title(
                "ARIMA Temperature Forecast", fontsize=14, fontweight="bold"
            )
        ...
        if hasattr(self, "future_predictions"):
            ...
            historical_annual = self.data.groupby("Year")[
                "Dhaka Temperature [2 m elevation corrected]"
            ].mean()
            ...
            axes[0, 1].set_title(
                "Annual Temperature Predictions", fontsize=14, fontweight="bold"
            )
        ...
        if "arima" in self.forecasts and "decomposition" in self.forecasts["arima"]:
            ...
            axes[1, 0].plot(
                trend_data.index[-120:],
                trend_data.values[-120:],
                color="green",
                linewidth=2,
            )
            axes[1, 0].set_title(
                "Temperature Trend Component (Last 10 Years)", fontweight="bold"
            )
        ...
        if "arima" in self.forecasts:
            ...
            summary_text += "ARIMA Model Results:\n"
            summary_text += f"• Model Order: {self.forecasts['arima']['order']}\n"
            summary_text += (
                f"• Seasonal Order: {self.forecasts['arima']['seasonal_order']}\n"
            )
            summary_text += f"• AIC: {arima_summary['aic']:.2f}\n"
            summary_text += f"• BIC: {arima_summary['bic']:.2f}\n\n"

            summary_text += "Temperature Predictions:\n"
            summary_text += (
                f"• Historical Avg: {arima_summary['historical_avg']:.2f}°C\n"
            summary_text += "ARIMA Model Results:\n"
            summary_text += f"• Model Order: {self.forecasts['arima']['order']}\n"
            summary_text += (
                f"• Seasonal Order: {self.forecasts['arima']['seasonal_order']}\n"
        ...
        if hasattr(self, "future_predictions"):
            if "arima" in self.future_predictions:
                ...
                summary_text += "Annual Forecasts:\n"
                for i, (year, temp) in enumerate(
                    zip(self.future_predictions["arima"]["years"], arima_temps)
                ):
    ...
    def get_prediction_summary(self):
        ...
        if "arima" in self.forecasts:
            ...
            summary += (
                f"  • Predicted Increase: {model_summary['forecast_increase']:.2f}°C\n"
            )
        ...
        if hasattr(self, "future_predictions"):
            ...
            if "arima" in self.future_predictions:
                arima_pred = self.future_predictions["arima"]
                summary += "ARIMA Annual Forecasts:\n"
                for year, temp in zip(arima_pred["years"], arima_pred["annual_temps"]):
                    increase = (
                        temp
                        - self.forecasts["arima"]["model_summary"]["historical_avg"]
                    )
        ...
        if "arima" in self.forecasts:
            summary += "ARIMA Model:\n"
            summary += "  • Type: Time Series Forecasting\n"
            summary += "  • Strengths: Captures trends and basic patterns\n"
            summary += f"  • Model Fit: AIC={self.forecasts['arima']['model_summary']['aic']:.1f}\n"

        if "sarima" in self.forecasts:
            summary += "\nSARIMA Model:\n"
            summary += "  • Type: Seasonal Time Series Forecasting\n"
            summary += "  • Strengths: Captures seasonal cycles and climate patterns\n"
            summary += f"  • Model Fit: AIC={self.forecasts['sarima']['model_summary']['aic']:.1f}\n"
            summary += "  • Seasonal Analysis: Advanced decomposition\n"
        ...
        if "lstm" in self.forecasts:
            ...
            summary += (
                f"  • Architecture: {lstm_perf['model_summary']['architecture']}\n"
            )
            summary += (
                f"  • Parameters: {lstm_perf['model_summary']['total_parameters']:,}\n"
            )
            summary += f"  • Sequence Length: {lstm_perf['model_summary']['sequence_length']} days\n"
            summary += (
                f"  • Features Used: {lstm_perf['model_summary']['features_used']}\n"
            )
        ...
        if "random_forest" in self.forecasts:
            ...
            if "feature_importance" in rf_perf:
                ...
                summary += (
                    f"  • Top Features: {', '.join(top_features['Feature'].values)}\n"
                )
        ...
        if "arima" in self.forecasts:
            ...
            current_heatwave_days = (
                self.data[self.data["Heatwave"]].groupby("Year").size().mean()
            )
            if increase > 0.5:
                projected_increase_factor = 1 + (increase * 0.3)  # Rough estimate
                projected_heatwave_days = (
                    current_heatwave_days * projected_increase_factor
                )
                summary += f"Expected heatwave increase: {(projected_increase_factor - 1) * 100:.0f}%\n"
                summary += (
                    f"Current avg heatwave days: {current_heatwave_days:.1f}/year\n"
                )
                summary += (
                    f"Projected avg heatwave days: {projected_heatwave_days:.1f}/year\n"
                )

        # Recommendations
        summary += "\n💡 RECOMMENDATIONS:\n"
        summary += f"{'=' * 25}\n"
        summary += "1. 🌡️  Implement heat early warning systems\n"
        summary += "2. 🌳 Accelerate urban reforestation programs\n"
        summary += "3. 🏠 Improve building cooling infrastructure\n"
        summary += "4. 📊 Enhance climate monitoring networks\n"
        summary += "5. 🎯 Develop heat adaptation strategies\n"
        summary += "6. 🔬 Continue regular model updates with new data\n"
